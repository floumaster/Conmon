...
const renderIcon = ({item, index}, chosenIconName, setChosenIconName, categoryColor) => {
    ...
    return (
        <TouchableOpacity
            style={chosenIconName === item ? [styles.newIconWrapper, {backgroundColor: categoryColor}] : styles.newIconWrapper}
...
const CategoryCreate = ({ navigation, route }) => {
    ...
    return (
        <View style={styles.wrapper}>
            <Tab
                headerTitle={categoryInfo ? "Exit the category" : "Create a new category"}
                style={{flex: 1}}
